...
class user_commands(commands.Cog):
    ...
    @app_commands.describe(user=discord.app_commands.locale_str("user_set_describe_user"), role=discord.app_commands.locale_str("user_set_describe_role"))
    @app_commands.rename(user=discord.app_commands.locale_str("user"), role=discord.app_commands.locale_str("role"),)
    @app_commands.choices(role=role_choices)
    async def user_set(self, interaction: discord.Interaction, user: discord.User, role: app_commands.Choice[str]):
        ...
        if author.stored and config["roles"][author.role]["permission_level"] >= 10:
            ...
            if role.value == "default":
                ...
                success_embed = discord.Embed(
                    ...
                    description=translator.translate(interaction.locale.value, "command.user.set.success_embed.description", user_id=user.id, role=role.value),
                    color=0x57F287)
